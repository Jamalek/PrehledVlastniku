		...
		try {
			...
			try {
				...
				try {
					...
					System.err.println("Číslo \""+cislo+"\" nebylo vepsáno do buňky");
				}
			}
		}
	}
		...
		try {
			...
			try {
				...
				try {
					...
					System.err.println("Řetězec \""+string+"\" nebyl vepsán do buňky");
				}
			}
		}
	}
...
//		}
	...
	private static void vypisNejvetsiVlastniky() {
		...
		row.createCell(1).setCellValue("podílová výměra");
		row.createCell(2).setCellValue("seznam LV");
		row.createCell(3).setCellValue("výměra LV m2");
	...
	public static void close() {
		try {
			FileOutputStream fileOut = new FileOutputStream(path.substring(0, path.lastIndexOf('.'))+" přehled vlastníků a parcel"+".xls");
